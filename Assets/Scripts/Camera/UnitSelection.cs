...
public class UnitSelection : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            selected = new List<AttackableObject>();
            foreach (AttackableObject u in playerUnits) if(inBounds(u.gameObject)) selected.Add(u);
            foreach (AttackableObject u in playerUnits)
            {
                ...
                u.SetSelected(inBounds(u.gameObject), selected.Count == 1);
            }
            isSelecting = false;
        }
        ...
        if (Input.GetMouseButtonDown(2) && selected.Count > 0 && !isSelecting)
        {
