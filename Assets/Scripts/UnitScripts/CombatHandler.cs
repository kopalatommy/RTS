...
//                                Debug.Log("Unit found target while overriding movement");
                                ...
                                if(!searcher.isCollector) searcher.isAttacking = true;
                            }
                            else
                            {
                                searcher.target = closest;
            ...
            for (int i = 0; i < unitCodes.Count; i++)
            {
                for (int j = 0; j < units[unitCodes[i]].Count; j++)
                {
                    ...
                    if (appr.isApproaching)
                    {
                        if (appr.overrideMovement)
                        {
                            dist = Vector2.Distance(new Vector2(appr.overrideMovePos.x, appr.overrideMovePos.z), new Vector2(appr.transform.position.x, appr.transform.position.z));
                            /*if (newDist < 1)
                            {
                            ...
                            else
                            {
                                if (dist < stoppingDist)
                                {
                                    ...
                                    //Set unit up for attacking
                                    appr.isApproaching = false;
                                    if (!appr.isAttacking) appr.startSearch = true;
                                    //appr.isAttacking = true;
                                    if (!appr.MoveToNextLoc() && appr.isCollector) appr.OnReachTarget();
                                }
                                else if (dist < 3)
                                {
                                    Debug.Log("Stopping dist: " + dist + " < " + stoppingDist);
                                }
                                /*else
                                {
                                    //Starting to move
...
public abstract class AttackableObject : MonoBehaviour
{
    ...
    internal List<Vector3> waypoints = null;
    ...
    public virtual void SetSelected(bool selected, bool only)
    {
